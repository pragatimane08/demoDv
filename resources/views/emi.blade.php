    ...
    <div class="card border-0 shadow-lg rounded-4 overflow-hidden" style="max-width: 850px; margin: auto;">
        <div class="card-header bg-golden py-4" style="background-color: #b8860b;">
            <div class="d-flex justify-content-between align-items-center">
                <h3 class="mb-0" style="color: white;">EMI Calculator</h3>
        ...
        <div class="card-body p-5">
            ...
            $inputs = [
                [
                    ...
                    'step' => 10000,
                    'default' => 2000000,
                    'marks' => [50000, 3000000, 6000000, 10000000]
                ],
                [
                    'label' => 'Interest Rate (%)',
                    'id' => 'interestRate',
            ...
            <div class="d-grid gap-3 d-md-flex justify-content-md-center mt-5">
                <button class="btn btn-lg px-5 rounded-pill glow-on-hover" onclick="calculateEMI()" style="background-color: #b8860b; border-color: #b8860b; color: white;">
...
<div id="resultSection" class="container py-5 d-none">
    <div class="card border-0 shadow-lg rounded-4 overflow-hidden" style="max-width: 850px; margin: auto;">
        <div class="card-header bg-golden py-4" style="background-color: #b8860b;">
            <div class="d-flex justify-content-between align-items-center">
                <h3 class="mb-0" style="color: white;">EMI Calculation Results</h3>
                <button class="btn btn-sm btn-light rounded-pill" onclick="showCalculator()">
                    <i class="bi bi-pencil-square me-1"></i> Recalculate
                </button>
            </div>
        </div>
        <div class="card-body p-5">
                        ...
                        <div class="row text-center">
                            <div class="col-md-4 mb-3 mb-md-0">
                                <div class="p-3">
                                    ...
                                    <h3 class="mb-0" style="color: #b8860b;">₹<span id="loanAmt">0</span></h3>
                                </div>
                            </div>
                            <div class="col-md-4 mb-3 mb-md-0">
                                <div class="p-3">
                                    <h6 class="text-muted mb-2">Total Interest</h6>
                                    <h3 class="mb-0" style="color: #28a745;">₹<span id="interestAmt">0</span></h3>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="p-3">
                                    <h6 class="text-muted mb-2">Total Payment</h6>
                                    <h3 class="mb-0" style="color: #8a6d3b;">₹<span id="totalPayment">0</span></h3>
                                </div>
                            </div>
                        </div>
                        <div class="row mt-4">
                            <div class="col-12 text-center">
                                <div class="p-3">
                                    <h6 class="text-muted mb-2">Monthly EMI</h6>
                                    <h2 style="color: #d9534f;">₹<span id="emiValue">0</span></h2>
                                </div>
                            </div>
                        </div>
                    </div>
...
function initializeSliders() {
    const sliders = ['loanAmount', 'interestRate', 'tenure'];
    ...
    sliders.forEach(id => {
        ...
        slider.addEventListener('input', function() {
            ...
            input.value = value;
            label.textContent = id === 'loanAmount' ? Number(value).toLocaleString('en-IN') : value;
        ...
        label.textContent = id === 'loanAmount' ? Number(defaultValues[id]).toLocaleString('en-IN') : defaultValues[id];
        track.style.setProperty('--fill-percent', `${initialPercent}%`);
    });
}

function calculateEMI() {
    const principal = parseFloat(document.getElementById('loanAmountSlider').value);
    ...
    document.getElementById('loanAmt').textContent = Math.round(principal).toLocaleString('en-IN');
    document.getElementById('interestAmt').textContent = Math.round(totalInterest).toLocaleString('en-IN');
    document.getElementById('totalPayment').textContent = Math.round(totalPayment).toLocaleString('en-IN');
    document.getElementById('emiValue').textContent = Math.round(emi).toLocaleString('en-IN');

    // Hide calculator and show results
    document.getElementById('calculator').classList.add('d-none');
    document.getElementById('resultSection').classList.remove('d-none');

    // Prepare data for chart (yearly breakdown)
    const labels = [];
    const principalData = [];
    const interestData = [];
    ...
    for (let year = 1; year <= years; year++) {
        ...
        principalData.push(Math.round(totalPrincipalPaid));
...
function adjustChartForMobile() {
    if (emiChart) {
        emiChart.options = getChartOptions();
        emiChart.update();
    }
}
